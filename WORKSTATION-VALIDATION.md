...
```sh
cd /path/to/draft-dsmullen-ppd-architecture
...
```powershell
cd C:\path\to\draft-dsmullen-ppd-architecture
...
- The rendered HTML output exists at `draft-dsmullen-ppd-architecture.html`.
- After `bootstrap --submission-tools`, `status` reports `Submission prerequisites: ready`.
- `validate-submission` completes and updates `versioned/draft-dsmullen-ppd-architecture-00.xml`.
